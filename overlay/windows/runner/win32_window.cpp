...
bool Win32Window::Create(const std::wstring& title,
  ...
  // Ghost overlay window styles:
  // WS_POPUP — frameless (no border/caption)
  ...
  // Extend DWM frame for glass-through transparency
  MARGINS margins = {-1, -1, -1, -1};
  DwmExtendFrameIntoClientArea(window, &margins);

  // Make window invisible to screen capture (Windows 10 2004+)
  typedef BOOL(WINAPI * SetWindowDisplayAffinityFunc)(HWND, DWORD);
  auto set_affinity = reinterpret_cast<SetWindowDisplayAffinityFunc>(
      GetProcAddress(GetModuleHandle(L"user32.dll"), "SetWindowDisplayAffinity"));
  if (set_affinity) {
    set_affinity(window, 0x00000011);  // WDA_EXCLUDEFROMCAPTURE
  }
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_MOUSEACTIVATE:
      // Prevent the window from activating on mouse click
