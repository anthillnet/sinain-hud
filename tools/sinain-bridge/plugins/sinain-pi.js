...
export function register(api) {
  try { api?.on?.('session.start', (event) => send('SessionStart', event)); } catch {}
  try { api?.on?.('session.end', (event) => send('Stop', event)); } catch {}
  try { api?.on?.('stop', (event) => send('Stop', event)); } catch {}
  try { api?.on?.('tool.call', (event) => send('PreToolUse', event)); } catch {}
  try { api?.on?.('tool.result', (event) => send('PostToolUse', event)); } catch {}
}
