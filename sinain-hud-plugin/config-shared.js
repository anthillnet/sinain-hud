...
export function writeEnv(vars) {
  // SECURITY: .env holds OPENROUTER_API_KEY — keep the dir and file owner-only.
  ...
  if (template) {
    ...
    fs.writeFileSync(ENV_PATH, template, { mode: 0o600 });
    fs.chmodSync(ENV_PATH, 0o600); // enforce even if the file pre-existed
  } else {
    const lines = [
...
export function writeAgentsConfig(patch) {
  ...
  // agents.json can carry gateway URLs/tokens — keep it owner-only.
  fs.writeFileSync(AGENTS_JSON_PATH, JSON.stringify(cfg, null, 2) + "\n", { mode: 0o600 });
  fs.chmodSync(AGENTS_JSON_PATH, 0o600); // enforce even if the file pre-existed
}
