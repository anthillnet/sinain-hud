...
sinain-core (Mac)                    OpenClaw Server
     ...
     │                                     │
     └── Session transcripts ────────────→ {{HISTORY_TOOL}} (queryable)
```
...
2. Call `{{SPAWN_TOOL}}` with the task
...
Use `{{HISTORY_TOOL}}` to read past sinain session transcripts and context.
...
{{HISTORY_TOOL}}({
  sessionKey: "{{SESSION_KEY}}",  // full session key (required)
...
|-----------|------|----------|-------------|
| `sessionKey` | string | **yes** | The session to query. For sinain: `"{{SESSION_KEY}}"` |
...
|---------------------|-------------------|
| User researching new tech/library | `{{SPAWN_TOOL}}`: "Deep-dive on [tech]. Quickstart, gotchas, best practices, alternatives." |
| Environment-related error (config, deps, infra) | `{{SPAWN_TOOL}}`: "Root cause analysis for [error]. Check known issues, changelogs, migration guides." |
| User asked a question in audio transcript | `{{SPAWN_TOOL}}`: "Find answer to: [question]. Summarize with sources." |
| User reading docs/tutorials on a topic | `{{SPAWN_TOOL}}`: "Compile key insights on [topic]. What matters most, common mistakes." |
...
4. **Non-blocking** — `{{SPAWN_TOOL}}` returns immediately; the subagent works in the background
...
| Spawn background work | `{{SPAWN_TOOL}}({task: "...", cleanup: "delete"})` |
| Read past context (light) | `{{HISTORY_TOOL}}({sessionKey: "{{SESSION_KEY}}", limit: 50, includeTools: false})` |
| Read past context (detailed) | `{{HISTORY_TOOL}}({sessionKey: "{{SESSION_KEY}}", limit: 10, includeTools: true})` — only when signals found |
| Proactive research | Respond first, then `{{SPAWN_TOOL}}` with research task (max 1 per escalation) |
