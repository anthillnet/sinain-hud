...
> **Execution contract:**
> 1. Setup → call `{{TICK_TOOL}}` → act on result → HEARTBEAT_OK
...
1. `{{HISTORY_TOOL}}({ sessionKey: "{{SESSION_KEY}}", limit: 20, includeTools: false })`
...
Call the `{{TICK_TOOL}}` tool:

```
{{TICK_TOOL}}({ sessionSummary: "...", idle: true|false })
```
...
- If `recommendedAction.action === "{{SPAWN_TOOL}}"` → spawn a subagent with `.task`
- If `recommendedAction.action === "telegram_tip"` → send `.task` to Telegram (chat_id: {{TELEGRAM_CHAT_ID}})
- If `output` is not null → send to Telegram (chat_id: {{TELEGRAM_CHAT_ID}}): `Suggestion: {suggestion}\n\nInsight: {insight}`
