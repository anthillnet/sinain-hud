...
> **Don't want to remember these?** **Right-click the eye** (macOS) for a native context menu of every action — enrich clipboard, select region, copy seed, reset position, settings, quit — with the shortcuts shown next to each, so the menu doubles as a cheat-sheet.
